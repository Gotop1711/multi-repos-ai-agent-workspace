...
`main`, then rebase the organization's branch.
...
- **"In use" is defined by the tracked tree, not by citations.** A
  ...
  product scope's folder had derivatives, the old repo-named scopes' copies
  did not.
...
- **`check` on `main` now warns about 13 orphans that are not orphans** —
  the originals of a scope whose derivatives live on the organization's
  branch only. Expected on this branch model (gitignored assets are shared
  across branches; derivatives are not) and harmless; the notice says
  "before closing out", and a closeout on `main` never touches another
  branch's scope folder. Worth remembering before "cleaning up" from
  `main`: check out that branch first.
