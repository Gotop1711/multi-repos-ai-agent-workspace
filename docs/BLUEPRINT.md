...
| A Changes ledger inside every scope document (rejected 2026-09-04, after two days of use) | Git history per file + session logs + this repo's `CHANGELOG.md` | Three ledgers recorded every event; every rebase conflicted at each ledger's anchor; readers paid for history on every session start. Git already keeps every prior version — the ledger only decided whether history was *also* kept in the live text |
| Struck-and-kept findings, or an `archive.md` for them (rejected 2026-09-04) | Delete once promoted, refuted or superseded; ids never reused; `git log -S` recovers the text | Readability for the reader and the agent is the constraint, not storage; an archive is the same rule in a different file |
